package com.nt.collectionenhancements;

import java.util.ArrayList;
import java.util.List;
import java.util.Random;
import java.util.Set;
import java.util.stream.Collectors;
import java.util.stream.Stream;

public class StreamExample {

	public static void main(String[] args) {
		Stream<Integer> stream= Stream.of(1,2,3,4);
		stream.forEach((x)->System.out.println(x));
		
		List<Employee> emp=new ArrayList<Employee>();
		emp.add(new Employee(1,"Nikhil",50050.033));
		emp.add(new Employee(2,"Raju",38000.0));
		emp.add(new Employee(3,"Rani",35400.0));
		emp.add(new Employee(4,"Kalyan",20000.0));
		
		//Write a program to print name whose name starts with ra without java 8
		for(int i=0;i<emp.size();i++) {
			if(emp.get(i).getEmpName().startsWith("Ra")) {
				System.out.println(emp.get(i).getEmpName());
			}
		}
		
		//Write a program to print name whose name starts with ra with java 8
		emp.stream().filter((x)->x.getEmpName().startsWith("Ra")).forEach((y)->System.out.println(y.getEmpName()));
		
		List<String> s1=new ArrayList<String>();
		s1.add("Nikhil");
		s1.add("Kiran");
		s1.add("vamshi");
		s1.add("Rajesh");
		
		s1.stream().forEach((x)->System.out.println(x));
		
		//write a program to collect emp names into separe list
		System.out.println("All names without java");
		List<String> emplo=new ArrayList<String>();
		for(int i=0;i<emp.size();i++) {
			String name=emp.get(i).getEmpName();
			emplo.add(name);
		}
		System.out.println(emplo.toString());
		
		System.out.println("All names with java streams");
		List<String> empName=emp.stream().map((x)->x.getEmpName()).collect(Collectors.toList());
		System.out.println("Names using streams: "+empName);
		
		
		List<Integer> numbers=new ArrayList<Integer>();
		numbers.add(10);
		numbers.add(20);
		numbers.add(30);
		numbers.add(33);
		numbers.add(55);
		numbers.add(60);
		
		numbers.stream().filter((x)->x%2==0).forEach((y)->System.out.println(y));
		Set<Integer> num=numbers.stream().filter((x)->x%2==0).collect(Collectors.toSet());
		System.out.println(num);
		
		//write a programm to collect all the emp ids to separate list
		List<Integer> ids=emp.stream().map((x)->x.getEmpId()).collect(Collectors.toList());
		System.out.println(ids);
		
		//generate
		Stream.generate(()->new Random().nextInt()).limit(4).forEach(p->System.out.println(p));
		Stream.generate(()->100).limit(4).forEach(p->System.out.println(p)); //we need to limit it or else it will loop continously.
	}
}
